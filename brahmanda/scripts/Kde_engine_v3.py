"""
kde_engine_v3.py  —  Bayesian Mixture Model with Scenario Separation
=====================================================================
...
def run_event_v3(
    ...
    base_quantile = ""
    if analogues:
        # Try to get base event quantile from test corpus
        # (not available directly — use price percentile as proxy)
        base_quantile = "40%-60%"   # default mid
...
def run_batch(analogues_dir: Path, prices: PriceData,
    ...
    log.info(f"\n{'='*60}")
    log.info(f"BATCH KDE v3  (Bayesian Mixture + Scenario Separation)")
    ...
    for jf in tqdm(json_files, desc="KDE-v3", unit=" ev"):
        try:
        ...
        out_file = per_dir / f"{event_id}_kde_v3.json"
    ...
    csv_path = output_dir / f"kde_v3_results_h{horizon}.csv"
...
def _print_summary(df: pd.DataFrame, horizon: int):
    ...
    print(f"\n{'='*65}")
    print(f"  KDE v3 RESULTS  (Bayesian Mixture + Scenario Separation)")
    ...
    if "covers_90" in valid.columns:
        ...
        print(f"\n  KDE v3 90% coverage    : {cov.mean()*100:.1f}%")
...
def main():
    ...
    ap.add_argument("--output",     default="kde_output_v3")
