...
real CLI signatures before being wired in here):
  ...
  2. garch_family_benchmark.py  -- EGARCH/GJR-GARCH comparison (Table 6-adjacent,
                             Figures 4-5)
  3. kde_baselines_v4.py    -- plain KDE / rolling KDE baselines (Table 2)
  4. compute_proper_scores_v4.py   -- CRPS, log score, DM tests (Table 6)
  5. compute_fei_by_event_type_v4.py -- divergence-based FEI (Table 13)
...
What this DOES NOT cover, and why:
  ...
  - Table 10 (regime breakdown by M1/M2 spread): NOT a separate script --
    ...
    kde_v4_results CSV ("base_spread_M1M2"), so it's generated inline
...
def main():
    ...
    ap.add_argument("--metadata", required=True,
        help="memory_2024_2026/df_major_metadata_2024_2026.csv "
             "(for compute_fei_by_event_type_v4.py's event-type labels)")
    ...
    ok = run_step(
        "kde_baselines_v4.py",
        [sys.executable, "kde_baselines_v4.py",
         "--results", str(kde_csv_h15),
         "--prices", args.prices,
         "--horizon", "15",
         "--output", str(out_dir / "kde_baselines")],
    )
    if not ok: failures.append("kde_baselines_v4.py")

    # ── Step 4: proper scoring rules (CRPS, log score, DM tests) ────────
    ok = run_step(
        "compute_proper_scores_v4.py",
        [sys.executable, "compute_proper_scores_v4.py",
         "--kde_dir", str(kde_dir),
         "--results", str(kde_csv_h15),
         "--output", str(out_dir / "proper_scores")],
    )
    if not ok: failures.append("compute_proper_scores_v4.py")

    # ── Step 5: FEI by event type (Table 13) ────────────────────────────
    ok = run_step(
        "compute_fei_by_event_type_v4.py",
        [sys.executable, "compute_fei_by_event_type_v4.py",
         "--kde_dir", str(kde_dir),
         "--results", str(kde_csv_h15),
         "--metadata", args.metadata,
         "--output", str(out_dir / "fei_by_event_type")],
    )
    if not ok: failures.append("compute_fei_by_event_type_v4.py")
    ...
    log.info("Next step: use the v4 outputs for the final Tables 2/4/6/7/10/11/12/13. "
              ...
              "divergence-based FEI from compute_fei_by_event_type_v4.py separate.")
