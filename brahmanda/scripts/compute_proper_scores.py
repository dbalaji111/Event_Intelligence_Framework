...
Computes distribution-level proper scoring rules for IJF Paper 1:
  ...
  1. CRPS (Continuous Ranked Probability Score)
     - For KDE v3: computed from stored x_grid and density via numerical integration
  ...
  2. Log Predictive Score (Log Score)
     - For KDE v3: log(f(y)) evaluated at realised return using stored density
...
def main():
    ...
    ap.add_argument("--kde_dir",  required=True,
                    help="kde_output_v3/per_event/")
    ap.add_argument("--results",  required=True,
                    help="figures/kde_v3_results_garch_family.csv")
    ...
    for _, row in tqdm(df.iterrows(), total=len(df),
        ...
        # Load KDE posterior
        json_path = kde_dir / f"{event_id}_kde_v3.json"
    ...
    for name, crps_col, logs_col in methods:
        ...
        marker = " ◄" if name == "KDE v3" else ""
        lines.append(f"  {name:<18} {c:>10.4f}  {l:>14.4f}{marker}")

    lines.append("  " + "-" * 50)
    lines.append(f"\n  DM tests on CRPS (KDE v3 vs benchmark):")
    ...
    lines.append("  (Negative DM = KDE v3 has lower CRPS = better)")
    lines.append("=" * 70)
    ...
    lines.append(r"DM = Diebold--Mariano test statistic (KDE v3 vs benchmark);")
    lines.append(r"negative values indicate KDE v3 is more accurate.")
    lines.append(r"$^{**}p<0.01$, $^*p<0.05$.}")
    lines.append(r"\label{tab:proper_scores}")
    lines.append(r"\begin{tabular}{lccc}")
    lines.append(r"\toprule")
    lines.append(r"Method & Mean CRPS & Mean Log Score & DM vs KDE v3 \\")
